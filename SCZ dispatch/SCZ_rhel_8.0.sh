#!/bin/sh
#
# Science Collaboration Zone (SCZ) Azure Linux Redhat 7.X automation bash script  V0.991
...
EOF
echo "realm join -v -U " $ARG1_UPPERCASE " " $DOMAIN
realm join -v -U $ARG1_UPPERCASE $DOMAIN >/dev/null 2>/dev/null <<EOF
`echo $2 | openssl enc -pbkdf2 -base64 -d -aes-256-cbc -salt -pass env:key`
EOF
#
# somehow we experience sometimes race conditions after a succesful join..... 
# so realm list will do ?
realm list
#
